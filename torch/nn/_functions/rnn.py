...
def ConditionedLSTMCell(input, context, hidden, w_ih, w_ch, w_hh, b_ih=None, b_ch=None, b_hh=None):
    hx, cx = hidden
    gates = F.linear(input, w_ih, b_ih) + F.linear(hx, w_hh, b_hh) + F.linear(context, w_ch, b_ch)
...
def AutogradRNN(mode, input_size, hidden_size, num_layers=1, batch_first=False,
    ...
    if mode == 'RNN_RELU':
        ...
        cell = RNNTanhCell
    elif mode == 'ConditionedLSTM':
        cell = ConditionedLSTMCell
    elif mode == 'LSTM':
        cell = LSTMCell
    elif mode == 'GRU':
        cell = GRUCell
