...
class ConditionedLSTMCell(RNNCellBase):
    r"""A long short-term memory (LSTM) cell conditioned on a context vector.
    ...
    def forward(self, input, context, hx):
        return self._backend.ConditionedLSTMCell(
            input, context, hx,
            self.weight_ih, self.weight_ch, self.weight_hh,
            self.bias_ih, self.bias_ch, self.bias_hh,
        )
